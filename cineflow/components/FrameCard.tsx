              ...
              <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => actions.removeFrame(fIdx)} className="w-10 h-10 flex items-center justify-center text-beige-400 hover:text-rose-500 transition-colors"><i className="fa-solid fa-trash"></i></button>
                    </div>
                  </div>
              </div>
                                ...
                                <>
                                  ...
                                  {img.status === 'completed' && (
                                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/img:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-4 p-8 backdrop-blur-md">
                                        <Button onClick={() => actions.selectImageForTransition(fIdx, iIdx)} variant="ghost" className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-xs ${frame.selectedImageIndex === iIdx ? '!bg-white !text-terracotta' : '!bg-white/20 !text-white'}`}> {frame.selectedImageIndex === iIdx ? 'Active Master' : 'Set as Master'} </Button>
                                        <div className="flex gap-2 w-full">
                                          <Button onClick={() => actions.setEditingImage({frameIdx: fIdx, imgIdx: iIdx})} variant="ghost" className="flex-1 py-4 rounded-xl font-bold uppercase tracking-widest text-xs text-white border border-white/20 hover:bg-white/10"> AI Refine </Button>
                                          <a 
                                              href={img.url} 
                      ...
                      <div className="flex-1 space-y-6">
                          <div className="flex justify-between items-center">
                              ...
                              <div className="flex gap-1.5 bg-beige-100 rounded-lg p-1 border border-beige-300">
                                  <button onClick={() => actions.updateFrameVideoModel('veo-3.1-fast-generate-preview')} className={`text-[9px] font-black uppercase px-4 py-1.5 rounded-md transition-all ${frame.videoModel?.includes('fast') ? 'bg-terracotta text-white shadow-sm' : 'text-beige-400'}`}>Fast</button>
                                  <button onClick={() => actions.updateFrameVideoModel('veo-3.1-generate-preview')} className={`text-[9px] font-black uppercase px-4 py-1.5 rounded-md transition-all ${!frame.videoModel?.includes('fast') ? 'bg-terracotta text-white shadow-sm' : 'text-beige-400'}`}>Premium</button>
                              </div>
                          </div>
                          
                          <div className="relative">
                              <button onClick={actions.handleGenFramePrompt} disabled={generatingPromptFor} className="absolute -top-3 right-4 bg-white px-3 text-[10px] font-black uppercase text-terracotta hover:text-terracotta-hover transition-colors flex items-center gap-2 border border-beige-200 rounded-full shadow-sm"> {generatingPromptFor ? <i className="fa-solid fa-spinner animate-spin"></i> : <i className="fa-solid fa-wand-magic-sparkles"></i>} AI Direct </button>
                              <textarea 
                          ...
                          <Button onClick={actions.generateFrameVideo} disabled={frame.videoStatus === 'generating' || !masterImage} className="w-full rounded-2xl py-5 text-xs font-black uppercase tracking-widest shadow-xl shadow-terracotta/10"> 
